...
public class Documento {
    ...
    // Método que obtiene los datos desde el archivo CSV
    ...
    // Método para verificar si un documento es mayor que otro
    ...
    // Método que ordena los datos según el algoritmo de QUICKSORT
    public static void ordenarRapido(int izq, int der, int criterio) {
        Documento pivote = documentos.get(izq);
        int i = izq;
        int j = der;

        while (i < j) {
            while (!esMayor(documentos.get(i), pivote, criterio) && i < j) i++;
            while (esMayor(documentos.get(j), pivote, criterio)) j--;
            if (i < j) intercambiar(i, j);
        }
        documentos.set(izq, documentos.get(j));
        documentos.set(j, pivote);
        if (izq < j - 1) ordenarRapido(izq, j - 1, criterio);
        if (j + 1 < der) ordenarRapido(j + 1, der, criterio);
    }

    // Método que ordena los datos según el algoritmo de INSERCIÓN
